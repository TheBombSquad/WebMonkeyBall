...
import type {
  ...
  MultiplayerGameMode,
  PlayerState,
  ResultReplayHistoryFrame,
} from './state.js';
import { computeGoalScore, normalizeGoalType } from '../rules/shared.js';
import type { Ruleset } from '../rules/ruleset.js';
import { getRulesetById } from '../rules/index.js';
...
function nlerpQuat(out: Quat, a: Quat, b: Quat, t: number) {
  let bx = b.x;
  let by = b.y;
  let bz = b.z;
  let bw = b.w;
...
export type GameCoreOptions = {
  ...
  stageParserId?: string | null;
  stageRulesetId?: string | null;
  audio?: AudioManager;
  ruleset?: Ruleset;
...
export class GameCore {
  ...
  private playersSortedCache: PlayerState[] | null;
  private playersSortedDirty: boolean;
  ...
  constructor({
    ...
    this.stageParserId = stageParserId ?? null;
    this.stageRulesetId = stageRulesetId ?? null;
    this.audio = audio ?? null;
    this.session = session ?? createSessionController();
    this.ruleset = ruleset ?? getRulesetById(this.stageRulesetId);
    ...
    this.ball = null;
    this.players = [];
    this.playersSortedCache = null;
    this.playersSortedDirty = true;
  ...
  init() {
    ...
    const localCamera = new GameplayCamera();
    this.players = [
      {
    ...
    this.cameraController = localCamera;
    this.running = true;
  }
  ...
  private invalidatePlayersSortedCache() {
    this.playersSortedDirty = true;
  }

  private getPlayersSorted() {
    if (!this.playersSortedDirty && this.playersSortedCache) {
      return this.playersSortedCache;
    }
    this.playersSortedCache = [...this.players].sort((a, b) => a.id - b.id);
    this.playersSortedDirty = false;
    return this.playersSortedCache;
  }

  getPlayersSortedCached() {
    return this.getPlayersSorted();
  }

  private syncRulesetFromStage() {
  ...
  private emitModHookBoolean<T extends keyof ModHooks>(name: T, payload: Parameters<NonNullable<ModHooks[T]>>[0]) {
    if (!this.modHooks.length) {
      return false;
    }
    let handled = false;
  ...
  private runAfterBallStepHooks(payload: Parameters<NonNullable<ModHooks['onAfterBallStep']>>[0]) {
    if (!this.modHooks.length) {
  ...
  private resolveSpawnPosition(
    ...
    if (!this.modHooks.length) {
      return this.cloneVec3(defaultPos);
    }
    for (const hooks of this.modHooks) {
      const handler = hooks?.onResolveSpawnPosition as ((ctx: any) => Vec3 | null | void) | undefined;
      if (!handler) {
        continue;
      }
      ...
      if (result) {
        return this.cloneVec3(result);
      }
    }
    return this.cloneVec3(defaultPos);
  }

  private emitSessionEvent(type: string, data?: unknown) {
    this.emitModHook('onSessionEvent', { game: this, type, data });
  }
  ...
  getMultiplayerDeterminismHash() {
    ...
    if (this.modHooks.length > 0) {
      for (const hooks of this.modHooks) {
        ...
        const modHash = handler({ game: this });
        if (modHash === undefined || modHash === null) {
          continue;
        }
        h = hashU32(h, Number(modHash) >>> 0);
      }
    }
    return h >>> 0;
  }
  ...
  loadRollbackState(state, { resetResultReplay = true }: { resetResultReplay?: boolean } = {}) {
    ...
    if (this.modHooks.length > 0) {
      ...
      this.emitModHook('onLoadState', { game: this, state, modState });
    }
  }
  ...
  private shouldCaptureResultReplayHistory() {
    ...
    if (!this.ruleset.shouldCaptureResultReplayHistory(this, localPlayer)) {
      return false;
    }
    ...
    const captureGoalTail = localPlayer.goalTimerFrames > 0
      ...
      && this.session.supportsGoalReplay(this);
  ...
  private initResultReplayCamera(replay: ActiveResultReplay, { secondaryFalloutCut = false }: { secondaryFalloutCut?: boolean } = {}) {
    ...
    if (replay.kind === 'fallout') {
      if (this.ruleset.resetFalloutReplayCamera) {
  ...
  private startResultReplay(
    ...
    if (!this.ruleset.canStartResultReplay(this)) {
      return false;
    }
    if (this.replayInputStartTick !== null || !this.autoRecordInputs) {
      return false;
  ...
  addPlayer(id: number, { spectator = false } = {}) {
    ...
    ball.playerId = id;
    const camera = new GameplayCamera();
    ...
    if (hasActiveStage && !pendingSpawn && !spectator) {
      ...
      const activePlayers = this.getPlayersSorted().filter((entry) => !entry.isSpectator && !entry.pendingSpawn);
      const spawnPlayers = [...activePlayers, player].sort((a, b) => a.id - b.id);
      const spawnPos = this.resolveSpawnPosition(player, spawnPlayers, startPos, startRotY, startPos);
  ...
  private getSpawnPositionForPlayer(
    ...
    const defaultPos = this.cloneVec3(startPos);
    return this.resolveSpawnPosition(player, activePlayers, startPos, startRotY, defaultPos);
  }
  ...
  getEffectRenderState(alpha = 1): EffectRenderState[] | null {
    ...
    const hasModEffects = this.modHooks.some((hooks) => !!hooks.onAppendEffectRender);
    if (effects.length === 0 && !hasModEffects) {
      this.renderEffects = null;
      return null;
    }
  ...
  async loadStage(stageId: number) {
    ...
    try {
      ...
      if (this.replayInputStartTick !== null) {
        ...
        const localPlayer = this.getLocalPlayer();
        if (localPlayer) {
          localPlayer.camera.initForStage(localPlayer.ball, localPlayer.ball.startRotY, this.stageRuntime);
        }
        this.replayAutoFastForward = true;
        this.setFixedTickMode(true, 1);
      }
  ...
  beginFalloutSequence(isBonusStage: boolean) {
    ...
    this.emitSessionEvent('fallout_start', { isBonusStage });
    this.goalReplayStartArmed = false;
    ...
    if (isBonusStage && this.session.isMultiplayer(this)) {
  ...
  beginTimeoverSequence(isBonusStage: boolean) {
    ...
    this.emitSessionEvent('timeover_start', { isBonusStage });
    this.goalReplayStartArmed = false;
    this.timeoverTimerFrames = TIMEOVER_TOTAL_FRAMES;
    this.multiplayerGoalTimerFrames = 0;
    if (this.session.isMultiplayer(this) && !isBonusStage) {
    ...
    localBall.speed = 0;
    if (!isBonusStage && this.lives > 0 && this.session.isSinglePlayer(this)) {
      this.lives -= 1;
    }
  ...
  beginBonusClearSequence() {
    const localPlayer = this.getLocalPlayer();
    const localBall = localPlayer?.ball ?? null;
    if (!localBall || !localPlayer || (localPlayer.goalTimerFrames > 0 && this.session.isSinglePlayer(this))) {
      return;
    }
    this.emitSessionEvent('bonus_clear_start');
  ...
  updateRingout(isBonusStage: boolean) {
    ...
    if (!isBonusStage && this.session.isSinglePlayer(this) && !this.activeResultReplay) {
    ...
    if (this.emitModHookBoolean('onRingoutComplete', { game: this, isBonusStage })) {
      return true;
    }
    if (isBonusStage) {
    ...
    this.accumulator = 0;
    if (this.session.isSinglePlayer(this) && this.stage) {
  ...
  async advanceCourse(info: { flags: number; goalType: string | null; timerCurr: number; u_currStageId: number }) {
    ...
    if (!advanced) {
      ...
      this.emitSessionEvent('course_complete', { info });
      return;
    }
    this.stageParserId = (this.course as any).currentStageParserId ?? this.stageParserId;
    this.stageRulesetId = (this.course as any).currentStageRulesetId ?? this.stageRulesetId;
  ...
  beginGoalSequence(goalHit: any) {
    ...
    this.emitSessionEvent('goal_start', { goalHit });
    ...
    this.goalReplayStartArmed = this.ruleset.shouldArmGoalReplay(this);
  }
  ...
  update(dtSeconds: number) {
    ...
    if (this.stageRuntime && this.stage && localBall && this.cameraController && this.world && localPlayer) {
      ...
      while (!this.pendingAdvance && this.accumulator >= this.fixedStep) {
        ...
        this.stageRuntime.goalHoldOpen = isSinglePlayer
          ? localPlayer.goalTimerFrames > 0
        ...
        for (const player of simPlayers) {
          ...
          if (player.goalTimerFrames > 0) {
            player.goalTimerFrames -= 1;
            if (player.goalTimerFrames <= 0) {
              player.goalTimerFrames = 0;
            }
          }
        ...
        for (const player of simPlayers) {
          if (player.spectateTimerFrames > 0) {
            ...
            if (player.spectateTimerFrames <= 0) {
              ...
              if (player.id === this.localPlayerId && !player.freeFly && isMultiplayer) {
                this.enterFreeFlyCamera(player);
              }
            }
          }
        ...
        const blockBonusClearForGoalSequence = isSinglePlayer && anyGoalSequenceActive;
        ...
        if (stageInputEnabled) {
          for (const player of simPlayers) {
            ...
            if (this.emitModHookBoolean('onGoalHit', {
              game: this,
              player,
              goalHit,
              resultReplayActive,
        ...
        if (isMultiplayer && !isBonusStage && !timeoverActive) {
